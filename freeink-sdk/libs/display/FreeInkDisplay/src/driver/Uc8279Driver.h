...
// UC8253 X3 — reuses BusyPolarity::X3TwoPhase.
...
class Uc8279Driver : public PanelDriver {
 public:
  ...
  void setBackgroundHint(bool darkBackground) { _darkBackground = darkBackground; }
 ...
 private:
  // Refresh split. Upstream exposes these through PanelDriver's async contract;
  // this SDK's PanelDriver has no such contract, so they stay internal and
  // display() runs them back to back.
  bool displayStart(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, RefreshMode mode, bool turnOff);
  void displayFinish(EpdBus& bus, const uint8_t* fb);
