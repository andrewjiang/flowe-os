...
uint8_t* ZipFile::readFileToMemory(const char* filename, size_t* size, const bool trailingNullByte) {
  ...
  if (fileStat.method == ZIP_METHOD_STORED) {
    ...
    // One-shot mode (no 32 KB ring buffer): the full output buffer is the
    // back-reference window, valid because we inflate in a single read().
    // Matters on the X3, where the ring alloc fails under BLE heap pressure
    // and every book showed "unreadable" (container.xml/content.opf reads
    // land here via readFileToMemory).
...
bool ZipFile::readFileToStream(const char* filename, Print& out, const size_t chunkSize) {
  ...
  if (fileStat.method == ZIP_METHOD_DEFLATED) {
    ...
    // Streaming inflate needs a 32 KB back-reference ring buffer. Under BLE
    // heap pressure on the X3 (~38 KB free, fragmented) that single block is
    // often unavailable, which used to fail every EPUB open ("unreadable").
    // Fallback: inflate the WHOLE file into one buffer (one-shot mode uses
    // the output itself as the window — no ring needed) and stream that out.
    // container.xml / content.opf are small, so the fallback covers the
    // metadata pass that decides readability.
    size_t outputBufferSize = chunkSize;
    bool oneShot = false;
    ...
    if (ctx.reader.init(true)) {
      ...
      if (outputBuffer) {
        LOG_DBG("ZIP", "Ring buffer unavailable; one-shot inflate of %zu bytes", outputBufferSize);
      }
    }
    if (!outputBuffer) {
      LOG_ERR("ZIP", "Failed to init inflate reader");
    ...
    while (true) {
      size_t produced;
      const InflateStatus status = ctx.reader.readAtMost(outputBuffer, outputBufferSize, &produced);
      // One-shot mode must produce everything in a single call; a second
      // call would reset dest_start and corrupt back-references.
      if (oneShot && status == InflateStatus::Ok) {
        LOG_ERR("ZIP", "One-shot inflate did not finish in a single pass");
        break;
      }
