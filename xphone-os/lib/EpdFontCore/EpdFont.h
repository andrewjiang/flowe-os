...
class EpdFont {
 ...
 public:
  ...
  // Interval-table membership only: true when the font natively carries cp.
  // Unlike getGlyph, never falls back to U+FFFD and never probes the
  // on-demand miss handler — coverage checks must be side-effect free.
