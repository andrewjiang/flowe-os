// Restored from x4-os lib/Epub/Epub/parsers/TocNcxParser.cpp (dropped in 622fc5f,
// "TOC parsing and TOC lookups (spine-linear R1)"). Wrapped in the reader
// namespace and pointed at ReaderLog, matching ContainerParser.
...
void XMLCALL TocNcxParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (self->state == IN_NAV_POINT && strcmp(name, "content") == 0) {
    ...
    if (!self->currentLabel.empty() && !self->currentSrc.empty()) {
      ...
      std::string href = fsutil::normalisePath(fsutil::decodeUriEscapes(rawPath));
      std::string anchor;

      if (pos != std::string::npos) {
        anchor = fsutil::decodeUriEscapes(rawTarget.substr(pos + 1));
      }
