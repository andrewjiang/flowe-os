// Pruned from x4-os lib/Epub/Epub/parsers/ContentOpfParser.cpp (403 LOC -> ~270).
...
// Case-insensitive "cover" substring for the last-resort href guess
// (covers "Cover.jpg" / "COVER.jpeg" naming in the wild).
bool hrefLooksLikeCover(const std::string& href) {
  if (href.size() < 5) return false;
  for (size_t i = 0; i + 5 <= href.size(); i++) {
    if (tolower(href[i]) == 'c' && tolower(href[i + 1]) == 'o' && tolower(href[i + 2]) == 'v' &&
        tolower(href[i + 3]) == 'e' && tolower(href[i + 4]) == 'r') {
      return true;
    }
  }
  return false;
...
void XMLCALL ContentOpfParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  // <meta name="cover" content="ID"> — points at a manifest item id (EPUB 2 convention)
  if (self->state == IN_METADATA && (strcmp(name, "meta") == 0 || strcmp(name, "opf:meta") == 0)) {
    ...
    if (isCover) {
      self->coverMetaItemId = coverItemId;
    }
    return;
  }
  ...
  if (self->state == IN_MANIFEST && (strcmp(name, "item") == 0 || strcmp(name, "opf:item") == 0)) {
    ...
    for (int i = 0; atts[i]; i += 2) {
      if (strcmp(atts[i], "id") == 0) {
        ...
        href = fsutil::normalisePath(fsutil::decodeUriEscapes(self->baseContentPath + atts[i + 1]));
    ...
    if (self->coverPropsHref.empty() && !properties.empty() && hasCoverImageProperty(properties)) {
      self->coverPropsHref = href;
    }

    // EPUB 2: item whose id the <meta name="cover"> points at. Some EPUBs aim
    // it at an XHTML wrapper — only trust it when the media-type is image/*.
    if (self->coverMetaHref.empty() && !self->coverMetaItemId.empty() && itemId == self->coverMetaItemId) {
      if (isImageMediaType(mediaType)) {
        self->coverMetaHref = href;
      } else {
    ...
    // Last resort: any image item whose href mentions "cover"
    if (self->coverGuessHref.empty() && isImageMediaType(mediaType) && hrefLooksLikeCover(href)) {
      self->coverGuessHref = href;
    }
    return;
  }
    ...
    if (self->state == IN_SPINE && (strcmp(name, "itemref") == 0 || strcmp(name, "opf:itemref") == 0)) {
      for (int i = 0; atts[i]; i += 2) {
        if (strcmp(atts[i], "idref") == 0) {
          ...
          // Linear scan (the large-manifest fast index was pruned for R1)
  ...
  // parse the guide for the "text" (start-of-content) reference
  if (self->state == IN_GUIDE && (strcmp(name, "reference") == 0 || strcmp(name, "opf:reference") == 0)) {
    ...
    for (int i = 0; atts[i]; i += 2) {
      if (strcmp(atts[i], "type") == 0) {
        ...
        guideHref = fsutil::normalisePath(fsutil::decodeUriEscapes(self->baseContentPath + atts[i + 1]));
      }
    }
    if (!guideHref.empty()) {
...
void XMLCALL ContentOpfParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (self->state == IN_MANIFEST && (strcmp(name, "manifest") == 0 || strcmp(name, "opf:manifest") == 0)) {
    self->state = IN_PACKAGE;
    self->tempItemStore.close();
    // Resolve the cover now that every manifest item has been seen.
    if (!self->coverPropsHref.empty()) {
      self->coverHref = self->coverPropsHref;
    } else if (!self->coverMetaHref.empty()) {
      self->coverHref = self->coverMetaHref;
    } else {
      self->coverHref = self->coverGuessHref;
    }
    if (!self->coverHref.empty()) {
      LOG_DBG("COF", "Cover item: %s", self->coverHref.c_str());
    }
    return;
  }
